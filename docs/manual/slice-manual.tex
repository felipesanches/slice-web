...
\begin{itemize}
  \item \textbf{Partial instancing is refused on fonts with variable kerning.} If a font carries a \texttt{GDEF} item variation store --- variable kerning and anchor positions --- Slice will pin its axes but will not narrow them, because rescaling the axis without rescaling that data would leave the kerning describing a design space the font no longer has. Measured against Google Fonts, this affects \textbf{52\% of the variable fonts that have an axis worth narrowing}. Pinning every axis works on essentially all of them. This is the largest gap in the program.
...
\begin{itemize}
  \item \textbf{\texttt{docs/test-suite.md}} --- all 297 conformance cases in plain English, each with the reasoning for why that is the right answer.
  \item \textbf{\texttt{docs/adjudication.md}} --- every case the original Slice fails, with the measurement behind each verdict.
  \item \textbf{\texttt{docs/real-world-sweep.md}} --- what happens on 775 real variable fonts from Google Fonts: 133,483 glyphs compared against fontTools with no disagreements.
  \item \textbf{\texttt{docs/original-behaviour.md}} --- the numbered map of the original program's behaviour that the test suite is written against.
\end{itemize}

\section{Credits and licence}

Slice is a reimplementation of \href{https://github.com/source-foundry/Slice}{Slice} by Source Foundry, which is the program this one is measured against throughout. The font engine is built on the \href{https://github.com/googlefonts/fontations}{fontations} crates; overlap removal uses \href{https://github.com/Logicalshift/flo_curves}{flo\_curves} and \href{https://github.com/linebender/kurbo}{kurbo}; the interface is \href{https://leptos.dev}{Leptos}.
