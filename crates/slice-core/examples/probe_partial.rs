...
fn dump_gvar(label: &str, font: &FontRef, gid: GlyphId) {
        ...
        for (i, m) in avar.axis_segment_maps().iter().enumerate() {
            ...
            let pairs: Vec<String> = m
                ...
                .map(|p| {
                    format!(
                        "{:.4}->{:.4}",
                        p.from_coordinate().to_f64(),
                        p.to_coordinate().to_f64()
                    )
                })
                .collect();
    ...
    let Ok(Some(data)) = gvar.glyph_variation_data(gid) else {
        println!("  no data for gid");
        return;
    };
    for (i, t) in data.tuples().enumerate() {
        let peak: Vec<f64> = t.peak().values().iter().map(|v| v.get().to_f64()).collect();
        let s: Option<Vec<f64>> = t
            .intermediate_start()
            .map(|x| x.values().iter().map(|v| v.get().to_f64()).collect());
        let e: Option<Vec<f64>> = t
            .intermediate_end()
            .map(|x| x.values().iter().map(|v| v.get().to_f64()).collect());
        let d: Vec<String> = t
            .deltas()
            .take(4)
            .map(|d| format!("{}:({},{})", d.position, d.x_delta, d.y_delta))
            .collect();
        println!(
            "  tuple {i}: peak={peak:?} inter={s:?}/{e:?} dense={} first={}",
            t.has_deltas_for_all_points(),
            d.join(" ")
        );
    }
}
...
fn main() {
    ...
    for p in &plans {
        println!(
            "plan {} limit {:?} normalized {:?}",
            p.spec.tag, p.limit, p.normalized
        );
    }
